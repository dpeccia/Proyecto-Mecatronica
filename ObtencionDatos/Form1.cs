...
namespace ObtencionDatos
{
    public partial class Form1 : Form
    {
        ...
        Boolean visualizacion = false; // Indicación de si el cuadro de visualización se encuentra abierto
        string estadoMecha;
        bool fin = false;
        bool archivoDrillOpen = false;
        bool archivoEsquinasOpen = false;
        int indexMecha = -1;
        ...
        private void AbrirToolStripMenuItem_Click(object sender, EventArgs e)//OK
        {
            ...
            if (archivoDrillOpen && archivoEsquinasOpen)
            {
                Graficador = new GraficadorForm(listaMechas, listaAgujeros, esquinas);
                archivoDrillOpen = false;
                archivoEsquinasOpen = false;
                VisualizarPuntos.Enabled = true;
            }
        }
        ...
        public void Recibir()
        {
            ...
            switch (estado[0])
            {
                case 'M':  // - Lo primero que envía el micro
                    if (indexAgujero != 0) Graficador.deseleccionarAgujero((Agujero)listaAgujeros[indexAgujero - 1]);
                    estadoMecha = "";
                    indexMecha++;
                ...
                case 'A':
                    ...
                    else if (numEsquina++ == 4)
                    {
                        ...
                        Graficador.seleccionarAjuste(extremoMin);
                    }
                    else
                    {
                        Enviar(extremoMax.xy);
                        Graficador.seleccionarAjuste(extremoMax);
                        numEsquina = 0;
                    }
                    break;
                case 'F': // - Lo tercero que envía el micro  
                    Graficador.deseleccionarAjuste(extremoMin);
                    Graficador.deseleccionarAjuste(extremoMax);
                ...
                case 'P': // - Orden de perforación
                    ...
                    if(visualizacion)
                    {
                        Graficador.seleccionarAgujero((Agujero)(listaAgujeros[indexAgujero]));
                    }
                    indexAgujero++;
                    break;
                case '*':
                    if (indexAgujero + 1 == listaAgujeros.Count)
                    {
                        ...
                        if (visualizacion)
                        {
                            Graficador.deseleccionarAgujero((Agujero)listaAgujeros[indexAgujero - 1]);
                            Graficador.seleccionarAgujero((Agujero)listaAgujeros[indexAgujero]);
                        }
                        Enviar("F");
                        fin = true;
